...
public:
   ...
   static int createCommandSocket( uint32_t inAddr, uint16_t port=commandPort )
   {
      ...
      // like 256 kB.
      const int rcvBufSize = 0x1E800;
      int sd;
      int tmp=0;
      socklen_t len= (socklen_t) sizeof(tmp); //0;
   ...
   static int createDataSocket( uint32_t inAddr, uint16_t port=dataPort, uint32_t multicastAddr=inet_addr("239.255.42.99") )
   {
      ...
      struct sockaddr_in localSock = /*createAddress(inAddr,port); */    createAddress(INADDR_ANY, port);
      
      if((sd = socket(AF_INET, SOCK_DGRAM, 0))  == -1)
      {
         printf("The data socket coul not be created");
         close(sd);
         return -1;
      }
      value = 1;
      ...
      if( (bind(sd, (struct sockaddr*)&localSock, sizeof(localSock))) == -1 )
      {
         printf("Failed to bind the data socket\n");
         close(sd);
         return -1;
      }
...
std::ostream& operator<<( std::ostream& s, Point3f const& point )
{
   ...
   << std::fixed
   << "( "
   << std::setprecision(6) << point.x << ", "
   << std::setprecision(6) << point.y << ", "
   << std::setprecision(6) << point.z << " )" << std::endl;
...
std::ostream& operator<<(std::ostream& s, MocapFrame const& frame)
{
   ...
   // Add code to display the markers that do not belong to the tracked rigid body.
   size = frame.unIdMarkers().size();
   for( i = 0; i < size; ++i ) {
     s << frame.unIdMarkers()[i] << std::endl;
   }
