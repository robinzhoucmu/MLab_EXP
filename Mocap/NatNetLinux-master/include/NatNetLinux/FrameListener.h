...
private:
   ...
   void _work(int sd)
   {
      printf(" Frame listener work started\n"); 

      NatNetPacket nnp;
      struct timespec ts;
      size_t dataBytes;
      
      fd_set rfds, exeptfds;
      struct timeval timeout;
      
      while(_run)
      {
        fprintf(stderr," Frame listener: work loop\n");
         ...
         FD_ZERO(&exeptfds); FD_SET(sd,&exeptfds);

         fprintf(stderr,"about to select \n");
         int selcRslt; // store the result of calling select
         if( ( selcRslt = select(sd+1, &rfds, 0, &exeptfds, &timeout)) <= 0 )
         {
            if(selcRslt == 0)
                 printf("(Time expired)continueuing ...\n");
            else
                  printf("(Select failed) continueing ... \n");
            continue;
         }
         fprintf(stderr,"Fram listener: finished select()\n");
         //clock_gettime( CLOCK_REALTIME, &ts );
          
         dataBytes = read( sd, nnp.rawPtr(), nnp.maxLength() );
         fprintf(stderr,"frame thread called read()");
